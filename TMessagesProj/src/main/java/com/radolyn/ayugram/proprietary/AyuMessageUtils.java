...
public abstract class AyuMessageUtils {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (tLRPC$PhotoSize != null) {
                            if ((tLRPC$PhotoSize instanceof TLRPC.TL_photoSize) && 
                                ...
                                ((bArr = tLRPC$PhotoSize.bytes) == null || bArr.length == 0)) {
        ...
        if (shouldSaveMedia(ayuSavePreferences)) {
            ...
            if (tLRPC$MessageMedia == null) {
                ...
                ayuMessageBase.documentType = 1;
                       ...
                       tLRPC$MessageMedia.document != null && 
                       (MessageObject.isStickerMessage(message) || 
                        message.media.document.mime_type.equals("application/x-tgsticker"))) {
                        ...
                        if (media != null && MessageObject.isVideoDocument(media.document)) {
                            ...
                            while (true) {
                                ...
                                if ((next instanceof TLRPC.TL_photoSize) && 
                                    (processAttachment = processAttachment(ayuSavePreferences.getAccountId(), next)) != null && 
                                    !processAttachment.getAbsolutePath().equals("/")) {
    ...
    private static boolean shouldSaveMedia(AyuSavePreferences ayuSavePreferences) {
        if (AyuConfig.saveMedia && ayuSavePreferences.getMessage().media != null) {
            ...
            TLRPC.Chat chat = MessagesController.getInstance(ayuSavePreferences.getAccountId())
                .getChat(Long.valueOf(Math.abs(ayuSavePreferences.getDialogId())));
            if (chat == null) {
